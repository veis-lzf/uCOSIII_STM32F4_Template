...
void BSP_Lcd_Init(void)
{
    ...
    lcd_dev.p_color = Red;
    Lcd_ColorBox(0, 0, lcd_dev.width, lcd_dev.heigth, lcd_dev.b_color); //必须初始化一次
...
u16 Read_ID(void)
{
    ...
    id = *(__IO u16 *) (Bank1_LCD_D);
    id = *(__IO u16 *) (Bank1_LCD_D);
    id1 = *(__IO u16 *) (Bank1_LCD_D);
    id2 = *(__IO u16 *) (Bank1_LCD_D);
    id = (id1 << 8) + id2;
    return id;
}
...
{
    Lcd_ColorBox(0, 0, lcd_dev.width, lcd_dev.heigth, color);
}
...
void LCD_showdouble(u16 x, u16 y, u16 width, u16 height, u8 size, double z)
{
    char str[20];
    sprintf(str, "%6.3f", z);
    ShowString(x, y, width, height, size, (uint8_t*)str);
}
