...
#if USMART_USE_HELP
			printf("------------------------USMART V3.1------------------------ \r\n");
			printf("    USMART一个灵巧的串口调试互交组件,是由ALIENTEK开发,由CJ移\r\n");
			printf("植到本工程,通过它,你可以通过串口助手调用程序里面的任何函数,\r\n");
			printf("以随意更改函数的输入参数(支持数字(10/16进制)、字符串、函数输入\r\n");	  
			printf("并执行.因此,你可口地址等作为参数),单个函数最多支持10个输入参\r\n");
			printf("数,并支持函数返 回值显示.新增参数显示进制设置功能,新增进制转\r\n");
			printf("换功能.\r\n");
			...
			if(i==0)//参数正常
			{
				...
				if(i==0)						   		//读取指定地址数据功能
				{
					...
					else
					{
						...
						if(usmart_dev.runtimeflag)printf("[DEBUG INF]:\tRun Time Calculation ON\r\n");
						else printf("[DEBUG INF]:\tRun Time Calculation OFF\r\n"); 
					}
...
{
	u8 sta,len;  
	if(USARTx_Read((USART_TypeDef*)D_Usmart_USART) == 1)//串口接收完成？
	{	          
		sta = usmart_dev.cmd_rec((u8*)(G_USARTx_Read_Buff+1));//得到函数各个信息
		if(sta == 0)usmart_dev.exe();	//执行函数 
		else 
		{   
			len = usmart_sys_cmd_exe((u8*)(G_USARTx_Read_Buff+1));
